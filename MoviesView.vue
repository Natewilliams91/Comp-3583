...
export default {
  data() {
    return {
      ...
      selectedDay: null,
      newTask: '',
      calendar: [], // Added the missing calendar data property
    };
  },
  mounted() {
  ...
  this.generateCalendar(this.currentYear, this.currentDate.getMonth());
    ...
    addTask() {
      if (!this.tasks[this.selectedFullDate]) {
        this.tasks[this.selectedFullDate] = []; // Initialize an array for the day if it doesn't exist
      }
      if (this.newTask.trim() !== '') {
        this.tasks[this.selectedFullDate].push(this.newTask); // Add the new task to the selected day
        this.newTask = ''; // Clear the input field
      }
    },
    removeTask(day, index) {
      if (this.tasks[this.selectedFullDate] && this.tasks[this.selectedFullDate].length > index) {
        this.tasks[this.selectedFullDate].splice(index, 1); // Remove the task at the specified index
      }
    },
  },
};
